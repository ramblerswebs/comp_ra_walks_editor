...
class WalkTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_ra_walks_editor.walk';
		parent::__construct('#__ra_walks_editor_walks', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if($array['date'] == '0000-00-00' || empty($array['date']))
		{
			$array['date'] = NULL;
			$this->date = NULL;
		}
		...
		if (!$user->authorise('core.admin', 'com_ra_walks_editor.walk.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_ra_walks_editor/access.xml',
				"/access/section[@name='walk']/"
			);
			$default_actions = Access::getAssetRules('com_ra_walks_editor.walk.' . $array['id'])->getData();
			$array_jaccess   = array();
